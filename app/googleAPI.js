...
// Force a fresh token on every full page load, never reuse a cached one from memory
let freshTokenRequiredOnLoad = true;
...
export async function requestDriveAppDataCredentials({ forcePrompt = true } = {}) {
  ...
  // Ensure a singleton token client
  if (!tokenClient) {
    tokenClient = window.google.accounts.oauth2.initTokenClient({
      client_id: oauth2_client_id,
      scope: SCOPES,
      callback: () => {
        // Will be set per request below
      },
    });
  }

  // If a token request is already in flight, reuse it to avoid races
...
export function setupGoogleDriveAppDataAuthOnLoad() {
  ...
  window.addEventListener(
    "load",
    () => {
  // No-op: avoid triggering a concurrent token request on load.
  // Token will be requested on first API use via ensureAccessToken().
    },
    { once: true }
  );
}
...
async function ensureAccessToken({ forcePrompt = false } = {}) {
  // If this is the first token request after a page reload, always request a new token
  if (freshTokenRequiredOnLoad) {
    freshTokenRequiredOnLoad = false;
    await requestDriveAppDataCredentials({ forcePrompt: true });
    return lastAccessToken;
  }
  if (lastAccessToken && !forcePrompt) return lastAccessToken;
